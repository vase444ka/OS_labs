...
namespace spos::lab1::demo {
    ...
    template<typename T>
    optional<T> gen_func(optional<pair<duration, T>> attribs) {
        if (attribs) {
    ...
    template<op_group O>
    typename op_group_traits<O>::value_type f_func(int case_nr) {
        return gen_func<typename op_group_traits<O>::value_type>(op_group_traits<O>::cases[case_nr].f_attrs).value();
    }

    template<op_group O>
    typename op_group_traits<O>::value_type g_func(int case_nr) {
